...
- Registers IPC handlers grouped by namespace: `memory:*`, `cost:*`, `claudeMd:*`, `sessions:*` (incl. `sessions:sendMessage`/`sessions:stopMessage` — continue an existing session via the **Agent SDK** `query({ options: { resume } })` (`modules/chat-runner.ts`), streaming over `sessions:chatChunk` (text deltas) + `sessions:chatMessage` (each fully-formed assistant/tool-result message as the SDK emits it, so the renderer builds the live turn — tools included — straight from the stream, not from a mid-stream disk re-read) + `chatDone`/`chatError`; and `sessions:startMessage` — start a *new* session via the SDK with a **pre-generated `sessionId`** (`crypto.randomUUID`), emitting `sessions:chatStarted` immediately (no race). Tool approvals go through the SDK's `canUseTool`: a non-auto-approved tool fires `sessions:permissionRequest` → the renderer's Allow/Always/Deny dialog → `sessions:permissionResponse` resolves the SDK's pending `PermissionResult` (`pendingPermissions` Map; `stopMessage`/supersede deny all pending). One `AbortController` in flight at a time), `rules:*`, `tasks:*`, `plans:*`, `agents:*`, `skills:*`, `mcp:*`, `projects:*` (duplicate detect/merge), `live:*` (Live Monitor), `ai:*`, `export:*`, `markdownFile:*`, `settings:*`, `config:*` (effective config via Agent SDK)
...
- `chat-runner.ts` — runs a chat turn through the Agent SDK `query()` (ESM dynamic import) instead of spawning `claude -p`, so it persists to the same `~/.claude/projects/<hash>/<id>.jsonl` (interchangeable with the terminal) **and** can request tool approvals interactively via `canUseTool`. Iterates the SDK message stream into flat `onStarted`/`onChunk` (text deltas)/`onMessage` (each fully-formed assistant or tool_result-bearing user message, mapped via `session-reader`'s shared `mapSdkMessageToChat`; skips the user message that merely echoes the prompt — the renderer already shows that optimistically)/`onError`/`onDone` callbacks; the SDK types are derived from the dynamic `import()` (no top-level `import type`, which the CJS build can't resolve). Also surfaces **slash-command output that carries no model turn**: a `system`/`compact_boundary` event (from `/compact`) and the final `result` text when no assistant message ran are emitted as synthetic assistant "note" messages via `onMessage` (`noteMessage`, marked `model: '<synthetic>'`). Local info commands like `/context`/`/usage` already arrive as their own `<synthetic>`-model assistant message (the real output) — but Claude Code **never persists it** (disk gets only the `"No response requested."` placeholder, filtered by `session-reader`), so the renderer pins the live `<synthetic>` output to keep it on screen (see ChatView). Slash commands run **natively** — the SDK executes a `/command` passed in the prompt string, no extra dispatch path. Backs `sessions:sendMessage` (resume) and `sessions:startMessage` (new, pre-generated `sessionId`)
