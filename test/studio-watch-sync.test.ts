...
describe('project workflow watcher sync', () => {
  it('refreshes once after a newly created transcript receives its cwd record', () => {
    ...
    const transcript = '/claude/projects/-Users-me-Acme2-0/session.jsonl';
    ...
    expect(invalidate).toHaveBeenCalledWith('-Users-me-Acme2-0');
  ...
  it('accepts a transcript written in the sessions/ layout', () => {
    ...
    expect(
      projectHashForRegistryEvent(
        '/claude/projects',
        '/claude/projects/-Users-me-Acme2-0/sessions/6f1e-8a2b.jsonl'
      )
    ).toBe('-Users-me-Acme2-0');
  });
  ...
  it('refreshes for a sessions/-layout project once its transcript is written', () => {
    ...
    coordinator.onEvent('/claude/projects/-Users-me-Acme2-0/sessions/6f1e-8a2b.jsonl');
    vi.advanceTimersByTime(100);
