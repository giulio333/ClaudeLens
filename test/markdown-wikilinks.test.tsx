...
describe('wikilink chips', () => {
  it('draws a resolved citation as a button carrying the path it found', async () => {
    resolvesTo({ 'Manuale Installazione.pdf': 'Progetti/Archivio/Manuale Installazione.pdf' });
    inChat('Fonte: [[Manuale Installazione.pdf]] in fondo alla nota.');

    const chip = await screen.findByRole('button', { name: 'Manuale Installazione.pdf' });
    expect(chip.className).toContain('cl-wikilink-found');
    expect(chip.getAttribute('title')).toBe('Progetti/Archivio/Manuale Installazione.pdf');

    chip.click();
    expect(bridge.api.vault.openFile).toHaveBeenCalledWith(
      ROOT,
      'Progetti/Archivio/Manuale Installazione.pdf'
    );
  });
  ...
  it('catches the backticked form too — the one in the screenshot', async () => {
    resolvesTo({ 'Scheda Modulo Delphi': 'Progetti/Scheda Modulo Delphi.md' });
    inChat('L’ambiente è pronto (`[[Scheda Modulo Delphi]]`).');

    const chip = await screen.findByRole('button', { name: 'Scheda Modulo Delphi' });
    expect(chip.className).toContain('cl-wikilink-found');
  });
