...
describe('parseShellCommand — statement splitting', () => {
  it('cuts a `;`-chained one-liner into one step per statement', () => {
    ...
    const cmd =
      'echo "=== LOG GRAFANA ==="; oc logs -n acme-monitoring deploy/grafana --tail=30 2>&1; echo; echo "=== OPERATOR ==="; oc get pod -n acme-monitoring 2>&1';
    expect(steps(cmd)).toEqual([
      'echo "=== LOG GRAFANA ==="',
      'oc logs -n acme-monitoring deploy/grafana --tail=30 2>&1',
      'echo',
      'echo "=== OPERATOR ==="',
      'oc get pod -n acme-monitoring 2>&1',
    ]);
  });
...
describe('parseShellCommand — pipelines', () => {
  ...
  it('breaks a long pipeline into its stages', () => {
    const cmd =
      'oc get pod -n acme-monitoring -l control-plane=controller-manager -o name 2>&1 | head -20';
    const parsed = parseShellCommand(cmd);
    expect(parsed.mode === 'steps' && parsed.steps[0].parts).toEqual([
      'oc get pod -n acme-monitoring -l control-plane=controller-manager -o name 2>&1',
      'head -20',
    ]);
  });
