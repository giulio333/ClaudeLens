...
describe('the census leaks no transcript content into the baseline', () => {
  ...
  it('collapses data-keyed maps instead of recording their keys', async () => {
    writeTranscript('s.jsonl', [
      ...
      },
      {
        ...
        uuid: 'f1',
        snapshot: {
          trackedFileBackups: { 'Ledger/CLAUDE.md': { x: 1 }, '/Users/someone/CV.html': { x: 2 } },
        },
      },
      {
        type: 'cost-state',
    ...
    for (const secret of ['10.1.2.3', 'Ledger', 'CV.html', 'someone', 'claude-opus-5']) {
      expect(
  ...
  it('collapses a map keyed by generated id, and stops above a foreign schema', async () => {
    ...
    writeTranscript('s.jsonl', [
      {
        ...
        wireIngestContext: {
          toolu_01H7XM6ap4zSvQmxLd1859nV: { cwd: '/Users/someone/Projects/Ledger' },
        },
      },
    ]);
    ...
    for (const secret of ['toolu_', 'someone', 'CV.html', 'Ledger', 'SECRET']) {
      expect(
        paths.filter(p => p.includes(secret)),
        `"${secret}" reached a key-path`
      ).toEqual([]);
