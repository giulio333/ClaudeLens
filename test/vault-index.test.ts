...
describe('resolveVaultLinks', () => {
  it('resolves a note cited without its extension', () => {
    write('Progetti/Scheda Modulo Delphi.md');
    expect(relOf('Scheda Modulo Delphi')).toBe('Progetti/Scheda Modulo Delphi.md');
  });

  it('resolves an attachment cited with its extension', () => {
    write('Progetti/Archivio/Manuale Installazione.pdf');
    const [answer] = resolveVaultLinks(root, ['Manuale Installazione.pdf']);
    expect(answer.rel).toBe('Progetti/Archivio/Manuale Installazione.pdf');
  ...
  it('answers a missing citation with null — the source is not there', () => {
    write('notes/Nota.md');
    const [answer] = resolveVaultLinks(root, ['Manuale Installazione.pdf']);
    expect(answer).toEqual({ target: 'Manuale Installazione.pdf', rel: null, match: null });
  });
  ...
  it('accepts a partial path only when exactly one file ends there', () => {
    write('Progetti/Archivio/Manuale.md');
    const [answer] = resolveVaultLinks(root, ['Archivio/Manuale']);
    expect(answer.rel).toBe('Progetti/Archivio/Manuale.md');
    expect(answer.match).toBe('fuzzy');

    write('Altro/Archivio/Manuale.md');
    resetVaultIndexCache();
    expect(relOf('Archivio/Manuale')).toBeNull();
  });

  it('does not fall back to a substring match', () => {
    write('notes/Manuale Installazione.md');
