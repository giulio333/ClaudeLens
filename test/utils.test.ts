...
describe('encodeProjectHash', () => {
  ...
  it('folds every non-alphanumeric character, not just the separators', () => {
    ...
    expect(encodeProjectHash('/Users/foo/Projects/Acme2.0')).toBe('-Users-foo-Projects-Acme2-0');
...
describe('resolveRealPath (cwd extraction)', () => {
  ...
  it('reads the cwd of a project whose transcripts live under sessions/', () => {
    ...
    const hash = project(
      [`${PREAMBLE}\n${JSON.stringify({ type: 'user', cwd: '/Users/foo/Acme2.0' })}\n`],
      'sessions'
    );
    expect(resolveRealPath(root, hash)).toBe('/Users/foo/Acme2.0');
