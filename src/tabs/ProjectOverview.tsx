...
import {
  GlobalClaudeMdView,
  ProjectClaudeMdView,
} from '../components/project/claudemd/GlobalClaudeMdView';
// ─── Skills
...
function viewForSection(section: ProjectSection, project: Project): View {
  switch (section) {
    case 'sessions':
      return { type: 'sessions', project };
    case 'memory':
      return { type: 'project-memory', project };
    case 'skills':
      return { type: 'project-skills', project };
    case 'agents':
      return { type: 'project-agents', project };
    case 'mcp':
      return { type: 'project-mcp', project };
    case 'live-agents':
      return { type: 'agents-live', project };
    case 'tasks':
      return { type: 'project-tasks', project };
    case 'plans':
      return { type: 'project-plans', project };
    case 'workflows':
      return { type: 'project-workflows', project };
    case 'teams':
...
export default function ProjectOverview() {
    ...
    async function loadSessions() {
      ...
      try {
        const rows = await Promise.all(
          projectsForSearch.map(async project => {
            try {
  ...
  function openSessionFromNotification(cwd: string, sessionId: string) {
    ...
    setView(
      sessionId
        ? { type: 'terminal', project, resumeSessionId: sessionId }
  ...
  function renderDeepView() {
    switch (view.type) {
      ...
      case 'skill-create':
        return (
          <CreateSkillPage
            ...
            onBack={() =>
              view.project
                ? setView({ type: 'project-skills', project: view.project })
                : setView({ type: 'global-skills' })
            }
            onSaved={() =>
              view.project
      ...
      case 'project-claudemd':
        return (
          <ProjectClaudeMdView layer={view.layer} onBack={() => setView({ type: 'overview' })} />
        );
      case 'chat':
        return (
          <ChatView
      ...
      case 'memory-topic':
        return (
          <MemoryTopicView
            ...
            hash={view.hash}
            onBack={() =>
              selected ? setView({ type: 'project-memory', project: selected }) : goGlobal()
            }
            onOpenSession={
              selected
      ...
      case 'analytics':
        return (
          <AnalyticsView project={view.project} onBack={() => setView({ type: 'overview' })} />
        );
      case 'ai-assistant':
        return (
          <AiAssistantView project={view.project} onBack={() => setView({ type: 'overview' })} />
        );
      case 'live-monitor':
    ...
    <div className="cl-app">
      ...
      <header className="cl-bar">
        ...
        <nav className="cl-scope">
          <button className={isGlobalHome ? 'on' : ''} onClick={goGlobal}>
            Global
          </button>
          <button
            className={scope === 'project' && !isGlobalLiveAgents ? 'on' : ''}
            onClick={goProjectScope}
          >
            Project
          </button>
          <button
            className={view.type === 'agents-live' && !view.project ? 'on' : ''}
            onClick={goLiveAgents}
          >
            Agent View
          </button>
          {STUDIO_ENABLED && (
