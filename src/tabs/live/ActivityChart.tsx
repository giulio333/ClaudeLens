...
function CustomTooltip({
  ...
  return (
    <div
      className="px-2.5 py-2 rounded-lg text-[9px]"
      style={{
    ...
    >
      <div className="mb-1 font-bold" style={{ color: '#4a5568' }}>
        {label}
      </div>
      {payload
        .filter(p => p.value > 0)
    ...
    <div className="relative h-full">
      {!hasData && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <span className="text-[10px] tracking-widest" style={{ color: '#2e3650' }}>
            — waiting for events —
          </span>
        </div>
      )}
