...
function getArg(input: Record<string, unknown>): string {
  if (input.file_path)
    return '…/' + String(input.file_path).split(/[\\/]/).filter(Boolean).slice(-2).join('/');
...
export default function LiveMonitor({
  ...
  const activeProcs = processes.filter(
    p => p.cwd === project.realPath || p.cwd.startsWith(project.realPath + '/')
  );
  ...
  return (
    ...
    >
      ...
      {/* ── Header ──────────────────────────────────────────────────────── */}
      <header
        className="relative z-10 shrink-0 flex items-center gap-3 px-5 py-2.5 border-b"
      ...
      >
        ...
        {(() => {
          const cfg = {
            idle: {
              ...
              color: '#4a5568',
              dot: '#4a5568',
              anim: undefined as string | undefined,
            },
            thinking: {
              label: 'THINKING',
              color: '#a855f7',
              dot: '#a855f7',
              anim: 'thinkPulse 1.6s ease-in-out infinite',
            },
            busy: {
              label: 'BUSY',
              color: '#00e5ff',
              dot: '#00e5ff',
              anim: 'busyPulse 1s ease-in-out infinite',
            },
          }[claudeStatus];
          return (
            <div
        ...
        <div className="w-px h-4" style={{ background: 'rgba(0,229,255,0.12)' }} />
        <span className="text-[12px] font-medium" style={{ color: '#787e98' }}>
          {projectName}
        </span>
        ...
        <div className="flex items-center gap-5">
          {[
            ...
            <div key={l} className="flex flex-col items-end">
              <span
                className="text-[7.5px] font-bold tracking-[0.16em]"
                style={{ color: '#2e3650' }}
              >
                {l}
              </span>
      ...
      <div className="relative z-10 flex overflow-hidden" style={{ flex: '1 1 0', minHeight: 0 }}>
        ...
        <aside
          className="w-[190px] shrink-0 flex flex-col border-r overflow-hidden"
        ...
        >
          ...
          >
            <div
              className="text-[7.5px] font-bold tracking-[0.22em] mb-3"
              style={{ color: '#2e3650' }}
            >
              PROCESS
            </div>
            {activeProcs.length === 0 ? (
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px]" style={{ color: '#2e3650' }}>
                  None detected
                </span>
              </div>
            ) : (
                ...
                <div key={p.pid} className="flex items-start gap-2">
                  <div className="relative shrink-0 mt-1">
                    ...
                    <div
                      className="absolute inset-0 rounded-full"
                      style={{
                        background: 'rgba(34,216,88,0.4)',
                        animation: 'ringPulse 2s ease-in-out infinite',
                      }}
                    />
                  </div>
                  <div className="min-w-0">
                    <div className="text-[11px] font-bold" style={{ color: '#22d858' }}>
                      PID {p.pid}
                    </div>
                    <div className="text-[9px] font-mono truncate" style={{ color: '#3d4460' }}>
                      {shortPath(p.cwd)}
                    </div>
                  </div>
                </div>
                  ...
                  .map(([name, count]) => {
                    ...
                    return (
                      <div key={name} className="flex items-center gap-2">
                        <span
                          className="text-[9.5px] font-mono truncate flex-1"
                          style={{ color: '#787e98' }}
                        >
                          {name}
                        </span>
                        <div
                          className="w-14 h-[3px] rounded-full overflow-hidden shrink-0"
                        ...
                        <span
                          className="text-[9.5px] font-bold tabular-nums w-4 text-right shrink-0"
                          style={{ color: '#00e5ff' }}
                        >
                          {count}
                        </span>
            ...
            <div className="flex flex-col items-center gap-4 text-center">
              ...
              <div
                ...
                style={{
                  ...
                  animation:
                    activeTool.status === 'running' ? 'glow 2s ease-in-out infinite' : undefined,
                }}
              >
                {activeTool.name}
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                {activeTool.status === 'done' && (
                  <span
                    ...
                    style={{
                      background: 'rgba(34,216,88,0.1)',
                      border: '1px solid rgba(34,216,88,0.2)',
                      color: '#22d858',
                    }}
                  >
                    DONE
        ...
        <aside
          className="w-[210px] shrink-0 flex flex-col border-l"
          style={{ borderColor: 'rgba(0,229,255,0.08)', background: 'rgba(3,4,10,0.65)' }}
        >
          <div
            ...
            style={{ borderColor: 'rgba(0,229,255,0.07)' }}
          >
            <span className="text-[7.5px] font-bold tracking-[0.22em]" style={{ color: '#2e3650' }}>
              PROJECT CONTEXT
            </span>
          </div>
      ...
      >
        ...
        <div className="absolute top-2 left-3 right-3 z-20 flex items-center gap-3 pointer-events-none">
          <span className="text-[7px] font-bold tracking-[0.25em]" style={{ color: '#2e3650' }}>
            ACTIVITY
          </span>
          <div className="flex items-center gap-3 ml-2">
            {(
              ...
              <div key={l} className="flex items-center gap-1">
                ...
                <span
                  className="text-[6.5px] font-bold tracking-widest uppercase"
