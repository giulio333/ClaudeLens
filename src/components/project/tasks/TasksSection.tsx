...
function GroupProgress({ tasks }: { tasks: TaskGroup['tasks'] }) {
  ...
  if (done) parts.push(<b key="d" className="d">{done} done</b>)
  if (running) parts.push(<b key="p" className="p">{running} in progress</b>)
  ...
  return (
    <section className="cl-section" style={{ paddingTop: 38 }}>
      <div className="cl-sec-head">
        <h2>Tasks</h2>
        <span className="ct">
          {groups.reduce((n, g) => n + g.tasks.length, 0)} across {groups.length} {groups.length === 1 ? 'session' : 'sessions'}
        </span>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="cl-task-legend">
          ...
          <span className="prog"><i />In progress</span>
          <span className="pend"><i />Pending</span>
        </div>
