...
| `LiveChatView.tsx` | `LiveChatView` | Vista **chat SDK in-app** (`View` case `new-chat`, ex `NewChatView`): la conversazione live, **interamente dallo stream, zero letture da disco** (display *e* contesto). Layout Focus con `ChatComposer`. Un `sessionMessages` in memoria parte vuoto e cresce **solo dallo stream**: a fine turno (`!streaming` dopo un send, guardato da `sawStreamingRef`) accoda bolla utente ottimistica (lo stream non riecheggia il prompt → uuid generato, riusato dalla bolla in volo) + i `liveMessages`, dedup per uuid; in volo `displayMessages` aggiunge la bolla ottimistica + `liveMessages`, con un `LiveTurn` finale per il testo assistant parziale (`onStreamChange`) o la chip "Using X…" (`onLiveToolChange`). **Una sola sessione SDK viva per tutta la chat**: il `ChatComposer` è montato **senza `key`** e riceve `sessionId` dopo `onStarted` → il 1° invio è `startMessage` (new), i successivi `sendMessage(sessionId)` **pushano nella query calda** (niente resume da disco). Non naviga mai via durante la chat (nessun remount/dispose; `endChat` solo all'uscita). `detailsFilter="minimal"`. Metadati live (costo/token/modello) dal `ChatTurnSummary` del `result` SDK (`onTurnComplete`), mostrati nella TopBar. `onSendFailed` ritira la bolla ottimistica se l'invio non parte. **Trade-off voluti**: niente continuazione di sessioni esistenti (→ terminale), niente export/highlights durante la chat (riapri la sessione read-only in `ChatView`). Ingressi: bottone "New chat" nell'hero e nell'header sezione Sessions di `ProjectOverviewContent` |
