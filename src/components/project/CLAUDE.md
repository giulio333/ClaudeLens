...
| `ChatComposer.tsx` | `ChatComposer` | Barra in basso al layout Focus. **Due modalità** in base a `sessionId`: presente → **continua una sessione esistente** (`sessions:sendMessage` → `claude -p --resume <sessionId>`, nuovi turni nello **stesso `.jsonl`**); assente → **avvia una nuova sessione** (`sessions:startMessage` → `claude -p` senza `--resume`, il nuovo id arriva su `onChatStarted` e viene propagato via prop `onStarted`). Due **selettori** nella meta-row (`ComposerSelect`, popover verso l'alto): **Model** (id ereditato in cima + alias Sonnet/Opus/Haiku + Default) e **Permission** (`default`/`acceptEdits`/`plan`/`bypassPermissions`, con `bypassPermissions` marcato in rosso via flag `danger`); entrambi passano a `--model`/`--permission-mode`. Lo stdout streamma live in una preview (`sessions:chatChunk`); al `chatDone` rifà `refetch()` del transcript. Enter invia, Shift+Enter newline; Stop → `sessions:stopMessage`. In `ChatView` è montato con `key={sessionId}` per ri-seedare lo state del picker al cambio sessione. La pill flottante si alza (`[data-composer]`) per far posto |
| `NewChatView.tsx` | `NewChatView` | Vista **nuova chat** (`View` case `new-chat`): layout Focus vuoto con `ChatComposer` in modalità new (niente `sessionId`). Al primo invio `claude -p` conia un nuovo session id (catturato via `onStarted`); al `chatDone` costruisce un `SessionSummary` minimale (`{id}.jsonl`, campi token/costo a zero, `firstUserMessage` dal testo inviato per il titolo) e lo passa a `onCreated` → naviga alla `chat` reale (il transcript è caricato da disco da `useChatSession`, il watcher rinfresca i metadati). Ingressi: bottone "New chat" nell'hero e nell'header sezione Sessions di `ProjectOverviewContent` |
