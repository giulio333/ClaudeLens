...
| `LivePulseBorder.tsx` | `LivePulseBorder` | Overlay decorativo **WebGL** (Paper Shaders `PulsingBorder`, dep `@paper-design/shaders-react`) per le superfici legate a una sessione live: bordo luminoso "che respira" con spot di colore in orbita sul perimetro. Assoluto sull'elemento parent (`position:relative` richiesto), `pointer-events:none`, `aria-hidden`. I `colors` accettano token `var(--cl-*)`/oklch — risolti a sRGB via probe canvas 2D e ri-risolti al flip del tema (`useTheme`); default sage+terracotta (`--cl-ok`/`--cl-accent`). Rispetta `prefers-reduced-motion` (non renderizza nulla: è decorazione, LED/LiveTag restano i segnali informativi); error boundary silenzioso interno. **Un contesto WebGL per istanza** (cap Chromium ~16/pagina): va montato solo su superfici live intrinsecamente poche, mai su liste unbounded. Usato da `overview/GlobalHomeView` (righe Live processes), `overview/ProjectOverviewContent` (riga sessione live) e `tabs/LiveMonitor` (cornice full-view legata allo status: cyan busy / viola thinking / spenta idle) |
