...
- **`PermissionRequestDialog.tsx`** — Dialog overlay (stile `SendConfirmDialog`) mostrato quando l'Agent SDK chiede l'approvazione di un tool via `canUseTool`. Mostra `toolName`, `title`/`displayName`, `description`, e il dettaglio rilevante dell'input (`command` per Bash, `file_path`/`path`, altrimenti JSON); `pendingCount` mostra quante richieste sono in coda dietro questa. Tre azioni: **Allow once** (`{ kind: 'allow' }`), **Always allow** (`{ kind: 'always', suggestions }`, solo se l'SDK fornisce `suggestions`) e **Deny…** (apre un textarea opzionale per il messaggio che Claude vedrà → `{ kind: 'deny', message }`). **`AskUserQuestion` ha un rendering dedicato** (`QuestionForm`): le domande di chiarimento di Claude (`input.questions[]`) come opzioni cliccabili (radio/checkbox per `multiSelect`) + campo "Other…" free-text; Answer risponde `{ kind: 'allow', input: { questions, answers } }` — le risposte viaggiano **dentro l'input approvato** (chiave = testo domanda, valore = label scelte joined ", " o testo libero), perché un Allow pass-through lascerebbe le domande senza risposta; Dismiss = deny. Input malformato → fallback al rendering generico. La decisione torna al main via `respondPermission(requestId, …)`. Tre cose l'SDK ora le **dichiara** invece di lasciarle indovinare (#292): `suppressAlwaysAllowRule` (la regola "Always allow" concederebbe più di questa richiesta) toglie il bottone e dice perché — e il main non inoltra nemmeno le `suggestions` e declassa un `always` arrivato comunque ad "allow once" (`electron/modules/chat-permissions.ts`), così il dialog non è l'unica serratura; `defaultToNo` apre il dialog col focus su **Deny…**, quindi Invio nega e non approva mai (nessuna scorciatoia approva: il focus è l'unico tasto in gioco); `mcpServer` stampa accanto al tool il server MCP e dove è configurato (`plugin`, `user`, `project`, …) — **come testo**, perché il nome viene dalla configurazione e React lo escapa, e nessuna decisione si prende da esso. Il dialog è montato per richiesta (`key={requestId}` in `ChatComposer`), quindi focus e testo del diniego non passano alla richiesta successiva in coda. Coperto da `test/permission-request-dialog.test.tsx` (StrictMode) e `test/chat-permissions.test.ts`
...
- **L'unità è il topic, non il file.** Su disco ricordare un fatto è sempre `Write` di `<slug>.md` **più** `Edit` di `MEMORY.md`: dentro una lista di file diventavano due righe di bookkeeping (`feedback_x.md +12 −0`, `MEMORY.md +1 −1`) che non dicevano mai _cosa_ è stato ricordato. Le operazioni sull'indice degradano a una nota `index MEMORY.md` in coda al blocco; i file di memoria sono esclusi da CHANGES **per path** (`memoryScopeOf` sul file, non sulla chiamata: un comando shell può riscrivere una memoria e un file del progetto insieme, e solo la prima è di MEMORY) per non contarli due volte nei totali di riga; una memoria **cancellata** resta in CHANGES, che dice DELETED, perché MEMORY non ha una parola per quello
- **Le letture stanno qui**, e sono l'unica traccia visibile di quali memorie hanno informato la sessione: un `Read` su un topic, e un **comando shell** — che è come l'auto mode legge e modifica la memoria (`cd …/memory && cat`, `sed -n`, `grep`). Le letture da shell passano dallo stesso lettore del rail del Lens (`shellReadPaths` in `chat/context-files.ts`, iniettato in `buildMemoryActivity` perché quel modulo dipende da questo), con le sue regole prudenti; le scritture da shell contano **solo dove Claude Code ha registrato un `bashEditDiff`** (creato → `new`, con diff → `revised`, elencato senza diff → `wrote`). Prima nessuna delle due entrava: una sessione che lavorava sulla memoria solo da shell non aveva nessuna riga MEMORY. **Cosa resta invisibile, di proposito**: il recall automatico (arriva come `<system-reminder>`, non passa da nessun tool) e una memoria riscritta da uno script senza diff registrato — `python3 - <<EOF … open(p, 'w')` non scrive nel transcript nulla che nomini il file, e leggere lo script per indovinarlo affermerebbe una modifica che il transcript non dice. Dalla 2.1.273 in poi, nei transcript osservati, una scrittura da shell nella cartella memory non porta mai il diff: una sessione così mostra le sue memorie come lette e basta
