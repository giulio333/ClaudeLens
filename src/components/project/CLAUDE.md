...
| `TeamsSection.tsx` | `TeamsSection` | Lista team via `useProjectTeams` in **slab card** (design direction 1c): slab scuro a sinistra (member count grande, dot colore per membro, stato `Live`/`Ended`/`Historical` — live via `isTeamLive`, dot pulsante), corpo editoriale a destra (kicker "Agent team", titolo `teamLabel`, meta mono `Lead <id8> · N transcripts · N messages · last activity`, azioni **Open chat ↗**/**Open team →**, chip membri `cl-team-chip` cap 6 + `+N`) e footer **token distribution** (barra segmentata per membro da `TeamSummary.memberTokens`, totale `fmtTokens`; nota "Configuration unavailable" quando `!hasConfig`). Card intera cliccabile → `team-detail` |
