...
| `useLiveTurn.ts` | `useLiveTurn`, `LiveTurnState`, `LiveTurnComposerHandlers` | **Hook: macchina a stati del turno live.** Incapsula `pendingUser`/`frozenMessages`/`liveMessages`/`pinnedSlash` + l'effetto di reconcile + il `weave` dello slash-command output, esponendo `displayMessages` (ciò che si renderizza), lo stato live (`liveText`/`liveTool`/`streaming`/`liveMessageCount`) e gli handler `composer` da cablare al `ChatComposer`. Logica anti-doppione (ignora il refetch a metà turno, riconcilia su disco a fine turno) + pinning dell'output `<synthetic>` dei comandi informativi keyed per UUID della command-card |
