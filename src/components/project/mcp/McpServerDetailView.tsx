...
export function McpServerDetailView({
  ...
  const scopeColor = fullyEnabled
    ? 'var(--cl-ok)'
    : fullyDisabled
      ? 'var(--cl-danger)'
      : 'var(--cl-warn)';
  const scopeLabel = fullyEnabled
    ? 'enabled everywhere'
    : fullyDisabled
      ? 'disabled everywhere'
      : 'partially enabled';
  ...
  return (
    <div className="h-full flex flex-col" style={{ background: 'var(--cl-paper)' }}>
      <TopBar
        onBack={onBack}
        crumbs={[{ label: 'Global · MCP' }, { label: displayName, accent: true }]}
      />
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!server.live && (
          <section className="cl-section">
            ...
            >
              The last <code style={{ fontFamily: 'var(--font-mono)' }}>claude mcp list</code> did
              not include this server, so{' '}
              <code style={{ fontFamily: 'var(--font-mono)' }}>/mcp</code> will not show it and its
              tools are unavailable. The name survives in{' '}
              <code style={{ fontFamily: 'var(--font-mono)' }}>~/.claude.json</code>, which keeps
              every connector ever connected to your account and never drops one you disconnect.
              Note that the list Claude Code reports varies between runs, so this reading can
              change.
            </div>
          </section>
        )}
        ...
        {(server.target || (isLocal && (commandLine || envKeys.length > 0))) && (
          <section className="cl-section">
            <div className="cl-sec-head">
              <h2>Configuration</h2>
              <span className="ct">
                {isLocal ? '~/.claude.json · settings.json' : 'claude mcp list'}
              </span>
            </div>
        ...
        {!isLocal && server.enabledProjectPaths.length > 0 && (
          <section className="cl-section">
            <div className="cl-sec-head">
              <h2>Enabled</h2>
              <span className="ct">
                {server.enabledProjectPaths.length}{' '}
                {server.enabledProjectPaths.length === 1 ? 'project' : 'projects'}
              </span>
            </div>
            <div>
        ...
        {server.disabledProjectPaths.length > 0 && (
          <section className="cl-section">
            <div className="cl-sec-head">
              <h2>Disabled</h2>
              <span className="ct">
                {server.disabledProjectPaths.length}{' '}
                {server.disabledProjectPaths.length === 1 ? 'project' : 'projects'}
              </span>
            </div>
            <div>
