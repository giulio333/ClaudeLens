...
function PanelHead({ meta, count }: { meta: { eyebrow: string; title: string; caption: string; ro: boolean }; count?: string }) {
  return (
...
export function GeneralTab({ cfg, heading }: { cfg: EffectiveConfig; q: string; heading?: boolean }) {
  const s = cfg.effective
  const init = cfg.init
  return (
    <>
      {cfg.initError && <div className="set-banner" style={{ marginTop: 24 }}>Runtime (init) info unavailable: {cfg.initError}</div>}
      {heading && <DomainLabel>General</DomainLabel>}
      <Block label="Runtime" grid>
        <Row k="Model" hint="Resolved model id for this scope">{init ? <Val>{init.model}</Val> : <Dim />}</Row>
        <Row k="Permission mode" hint="Effective at startup, after the trust filter">{init ? <Pill>{init.permissionMode}</Pill> : <Dim />}</Row>
        ...
        <Row k="Required version" hint="Minimum Claude Code for this ClaudeLens"><Val>{claudeCodeVersion}</Val></Row>
        <Row k="Working directory" stack full>{init ? <Val sm>{init.cwd}</Val> : <Dim />}</Row>
      </Block>

      <Block label="Preferences" grid>
        <Row k="Theme" src={prov(cfg, 'theme')}>{val(s.theme) ?? <Dim>system default</Dim>}</Row>
        <Row k="Configured model" src={prov(cfg, 'model')}>{val(s.model) ?? <Dim>default</Dim>}</Row>
        <Row k="Language" src={prov(cfg, 'language')}>{val(s.language) ?? <Dim>default</Dim>}</Row>
        <Row k="Effort level" src={prov(cfg, 'effortLevel')}>{val(s.effortLevel) ?? <Dim>default</Dim>}</Row>
        <Row k="Transcript retention" src={prov(cfg, 'cleanupPeriodDays')}>
          {s.cleanupPeriodDays != null ? <Val>{String(s.cleanupPeriodDays)} days</Val> : <Dim>30 days (default)</Dim>}
        </Row>
      </Block>
    </>
  )
...
export function PermissionsTab({ cfg, q, heading }: { cfg: EffectiveConfig; q: string; heading?: boolean }) {
  ...
  return (
    <>
      ...
      <Block label="Mode" grid>
        <Row k="Default mode" src={prov(cfg, 'permissions')}><Pill>{String(p.defaultMode ?? 'default')}</Pill></Row>
        {cfg.init && <Row k="Effective at startup" hint="After the CLI trust filter"><Pill>{cfg.init.permissionMode}</Pill></Row>}
      </Block>
      <Block label="Rules">
        ...
        <RuleRow label="Additional directories" items={filterList(p.additionalDirectories as string[], q)} />
      </Block>
    </>
  )
}

export function ToolsTab({ cfg, q, heading }: { cfg: EffectiveConfig; q: string; heading?: boolean }) {
  ...
  return (
    <Block label={heading ? 'Tools' : undefined} bare={!heading} count={heading ? all.length : undefined}>
...
export function ExtensionsTab({ cfg, q, heading }: { cfg: EffectiveConfig; q: string; heading?: boolean }) {
  const init = cfg.init
  const filt = (arr: string[] = []) => arr.filter(x => !q || x.toLowerCase().includes(q))
  return (
    <>
      {heading && <DomainLabel>Extensions</DomainLabel>}
      <Block label="Skills" count={init?.skills.length}><InvList items={filt(init?.skills)} empty={!init} /></Block>
      <Block label="Subagents" count={init?.agents.length}><InvList items={filt(init?.agents)} empty={!init} /></Block>
      <Block label="Slash commands" count={init?.slashCommands.length}><InvList items={filt(init?.slashCommands)} empty={!init} /></Block>
      <Block label="Plugins" count={init?.plugins.length}>
        {!init ? <p className="set-dim">Runtime info unavailable.</p> : (init.plugins?.length ?? 0) === 0 ? <p className="set-dim">None.</p> : (
          <div>
            {init.plugins.filter(p => !q || p.name.toLowerCase().includes(q)).map(p => (
              <div key={p.name} className="set-line">
                ...
                <span className="set-val sm" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.path}</span>
              </div>
            ))}
          </div>
        )}
    ...
    <>
      <Block label="Usage analytics">
        <p className="set-block-hint">
          Helps us understand how many people use ClaudeLens and on which platforms. Data is anonymous, aggregate, and never identifies you.
        </p>
        <div className="set-card">
          <div className="min-w-0">
            <div style={{ fontSize: 13.5, color: 'var(--cl-ink)', fontWeight: 600 }}>Share anonymous usage data</div>
          ...
          <ToggleSwitch checked={on} disabled={isLoading || setEnabled.isPending} onChange={v => setEnabled.mutate(v)} />
        </div>
      </Block>

      <Block label="Collected">
        <ul className="set-ledger">{COLLECTED.map((x, i) => <LedgerItem key={i} tone="neutral">{x}</LedgerItem>)}</ul>
      </Block>
      <Block label="Never collected">
        <ul className="set-ledger">{NOT_COLLECTED.map((x, i) => <LedgerItem key={i} tone="block">{x}</LedgerItem>)}</ul>
      </Block>

      <Block label="Details" grid>
        <Row k="Processor" hint="Privacy-first analytics for desktop apps">
          <a href="https://aptabase.com" target="_blank" rel="noreferrer" className="set-val" style={{ color: 'var(--cl-accent)' }}>Aptabase (EU)</a>
        </Row>
        <Row k="Legal basis" hint="GDPR Art. 6(1)(f) — lawful because the data is anonymous"><Val>Legitimate interest</Val></Row>
        <Row k="Privacy policy">
          <a href={PRIVACY_POLICY_URL} target="_blank" rel="noreferrer" className="set-val" style={{ color: 'var(--cl-accent)' }}>Read PRIVACY.md →</a>
        </Row>
      </Block>
    </>
  )
...
function ToggleSwitch({ checked, disabled, onChange }: { checked: boolean; disabled?: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      style={{ width: 44, height: 26, background: checked ? 'var(--cl-accent)' : 'var(--cl-ink-4)', opacity: disabled ? 0.5 : 1, cursor: disabled ? 'default' : 'pointer' }}
    >
      <span className="absolute rounded-full transition-all" style={{ top: 3, left: checked ? 21 : 3, width: 20, height: 20, background: 'var(--cl-paper)', boxShadow: '0 1px 2px rgba(0,0,0,0.25)' }} />
    </button>
  )
}
...
function DomainLabel({ children }: { children: ReactNode }) {
  return <h2 className="set-title" style={{ fontSize: 21, marginTop: 30 }}>{children}</h2>
}

function Block({ label, hint, bare, grid, count, children }: { label?: string; hint?: string; bare?: boolean; grid?: boolean; count?: number; children: ReactNode }) {
  return (
...
function Row({ k, hint, src, stack, full, children }: { k: string; hint?: string; src?: SrcInfo; stack?: boolean; full?: boolean; children: ReactNode }) {
  return (
...
function RuleRow({ label, items, tone }: { label: string; items?: string[]; tone?: 'ok' | 'warn' | 'danger' }) {
  const list = items ?? []
  return (
    <div className="set-row">
      <div className="set-row-line" style={{ alignItems: 'baseline' }}>
        ...
        <span className="leader" />
        <span className="v"><span className="set-val sm">{list.length || 'none'}</span></span>
      </div>
      {list.length > 0 && (
        <div className="set-cloud" style={{ marginTop: 10 }}>
          {list.map((x, i) => <span key={i} className={`set-chip${tone ? ` ${tone}` : ''}`}>{x}</span>)}
        </div>
      )}
    </div>
  )
...
function InvList({ items, empty }: { items: string[]; empty?: boolean }) {
  ...
  return <div className="set-inv">{items.map((x, i) => <span key={i} className="set-inv-item" title={x}>{x}</span>)}</div>
}

function RuntimeUnavailable({ label }: { label?: string }) {
  return <Block label={label} bare={!label}><p className="set-dim">Runtime info unavailable.</p></Block>
}
...
export function ReadOnlyHint() {
  return (
    <div className="set-foot">
      ...
      <span>
        Read-only. To change these, run <code className="font-mono" style={{ color: 'var(--cl-ink-3)' }}>/config</code> in Claude Code or edit your <code className="font-mono" style={{ color: 'var(--cl-ink-3)' }}>settings.json</code> files. The source stamp on each value shows which file it came from.
      </span>
    </div>
  )
}
