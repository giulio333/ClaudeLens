...
import { AGENT_OPTION_DEFS, readOptions, serializeAgent, initialOf } from '../shared/entityOptions';
...
export function AgentDetailView({
  ...
  const scope =
    agent.scope === 'global' ? 'Global' : agent.scope === 'plugin' ? 'Plugin' : 'Project';
            ...
            agent.missingRequired.length > 0 && (
              <>
                ...
                {agent.missingRequired.map((f, i) => (
                  <span key={f}>
                    <code className="font-mono" style={{ fontSize: 12, color: 'var(--cl-warn)' }}>
                      {f}
                    </code>
    ...
    <EntityDetailView
      ...
      onDelete={
        readOnly
          ? undefined
          : async () => {
              await del.mutateAsync({ filePath: agent.path });
            }
      }
      renderRunOverlay={
        canRun
