...
export function GlobalClaudeMdView({ onBack }: { onBack: () => void }) {
  ...
  return (
    <EntityDetailView
      ...
      onBack={onBack}
      onSave={async next => {
        await write.mutateAsync(next);
      }}
      onDelete={async () => {
        await del.mutateAsync(null);
      }}
    />
  );
}
...
export function ProjectClaudeMdView({
  ...
  return (
    <EntityDetailView
      ...
      onBack={onBack}
      onSave={async next => {
        await write.mutateAsync({ filePath: layer.filePath, content: next });
      }}
      onDelete={async () => {
        await del.mutateAsync(layer.filePath);
      }}
    />
  );
}
