...
export function buildProcessedMessages(messages: ChatMessage[]): ProcessedMessage[] {
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (msg.role === 'user') {
      const text = msg.content.find(b => b.type === 'text') as
        Extract<ChatContentBlock, { type: 'text' }> | undefined;
      if (text) {
    ...
    const toolUseBlocks = msg.content.filter(b => b.type === 'tool_use') as Extract<
      ChatContentBlock,
      { type: 'tool_use' }
    >[];
...
export type TurnVariant =
  'user' | 'claude' | 'agent' | 'command' | 'skill' | 'question' | 'plan' | 'notification';
...
export function describeTurn(
  ...
  const isPlanTurn = showPlanStrip && textBlocks.length === 0 && !showAgentStrip && !showQuestions;
  ...
  const variant: TurnVariant = isCommandTurn
    ? isSkillTurn
      ? 'skill'
      : 'command'
    : isQuestionTurn
  ...
  const color =
    variant === 'agent'
      ? (agentColor?.(
          (agentGroups[0]?.use.input as Record<string, unknown>)?.subagent_type as string
        ) ?? meta.color)
      : meta.color;
    ...
    if (c) {
      ...
      if (c.isSkill || skill) {
        out.push({
          ...
          description:
            skill?.description ?? (c.description !== 'Claude Code command' ? c.description : ''),
          scope: skill?.scope,
          skill,
        });
      }
...
export const MEMORY_TYPE_STYLE: Record<MemoryType, { badge: string; label: string }> = {
  ...
  feedback: {
    badge: 'bg-amber-950/20 text-amber-400 border border-amber-700/40',
    label: 'Feedback',
  },
  project: {
    badge: 'bg-emerald-950/20 text-emerald-400 border border-emerald-700/40',
    label: 'Project',
  },
  reference: {
    badge: 'bg-violet-950/20 text-violet-400 border border-violet-700/40',
    label: 'Reference',
  },
};
