...
export function SessionGraphView({ processed, onSelectTool }: Props) {
  ...
  const filteredLanes = useMemo(() => {
    ...
    return model.lanes.filter(lane => {
      ...
      if (q && !lane.label.toLowerCase().includes(q) && !(lane.fullPath ?? '').toLowerCase().includes(q)) {
        return false
      }
      return true
    })
      ...
      <div className="cl-tl-filters">
        ...
        <div className="cl-tl-filters-r">
          <div className="cl-tl-meta">
            <b>{filteredLanes.length}</b>/{model.lanes.length} lanes · <b>{filteredTools}</b>/{totalTools} tools · {fmtDuration(duration)}
          </div>
          <div className="cl-tl-zoom" role="group" aria-label="Zoom">
            <button type="button" onClick={() => setZoom(z => Math.max(1, z * 0.8))} title="Zoom out">−</button>
            <span className="cl-tl-zoom-val">{Math.round(zoom * 100)}%</span>
            <button type="button" onClick={() => setZoom(z => Math.min(20, z * 1.25))} title="Zoom in">+</button>
            <button type="button" onClick={() => setZoom(1)} title="Reset zoom" className="cl-tl-zoom-reset">⤺</button>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <UserMarkers
              ...
              onHover={(m) => {
                if (!m) handleUserHover(null)
                  ...
                  const x = scale(m.t)
                  handleUserHover(m, x + (wrapRef.current?.getBoundingClientRect().left ?? 0) - (scrollRef.current?.scrollLeft ?? 0) + LANE_LABEL_WIDTH, HEADER_HEIGHT)
                }
              }}
            />
            ...
            {filteredLanes.map((lane, i) => (
              <LaneRowWrap
                ...
                scale={scale}
                totalWidth={svgWidth}
                selected={selectedLane === lane.id || selectedLane === null ? selectedLane === lane.id : false}
                onSelectLane={setSelectedLane}
                ...
                setHoveredEventId={(id) => {
                  ...
                  if (!id) handleEventHover(null)
                    ...
                    handleEventHover(
                      ...
                      lane,
                      {
                        left: (wrapRect?.left ?? 0) + LANE_LABEL_WIDTH + (x - scrollLeft) - LANE_LABEL_WIDTH,
