...
export function ModelChip({ model, pct }: { model: string; pct: number }) {
  ...
  return (
    <div
      className="relative flex items-center gap-2 rounded-lg border border-[var(--cl-line)] bg-[var(--cl-paper-2)] px-3 py-1.5 overflow-hidden"
    >
      <div
      ...
      <span className="relative text-[11px] font-mono text-[var(--cl-ink-3)]">{pct}%</span>
    </div>
  )
}
