...
function mcpDetail(server: McpServer, totalProjects: number): string {
  if (server.source === 'local') {
    ...
    if (server.command) return `${server.command}${server.args?.length ? ` ${server.args.join(' ')}` : ''}`
    return envCount > 0 ? `${envCount} env vars` : 'local server'
  }
  const denom = totalProjects > 0 ? totalProjects : server.enabledInProjects + server.disabledInProjects
...
export function SearchPopover({
  ...
  const { sections, flat, counts } = useMemo(() => {
    ...
    const projectRows = sortedByName(projects.filter(matchesProject), projectName).map((p): ProjectRow => {
    ...
    const skillRows: EntityRow[] = sortedByName(
      skills.filter(s => includesQuery(q, [s.name, s.description, s.path, s.scope, s.model, s.agent])),
      s => s.name,
    ).map(s => ({
      kind: 'skill',
      key: `skill:${s.path}`,
    ...
    const agentRows: EntityRow[] = sortedByName(
      agents.filter(a => includesQuery(q, [a.name, a.description, a.path, a.scope, a.model, ...(a.skills ?? []), ...(a.mcpServers ?? [])])),
      a => a.name,
    ).map(a => ({
      kind: 'agent',
      key: `agent:${a.path}`,
    ...
    const mcpRows: EntityRow[] = sortedByName(
      mcpServers.filter(s => includesQuery(q, [
        s.name,
        mcpDisplayName(s),
        s.source,
        s.command,
          ...
          {FILTERS.map(f => {
            ...
            return (
              <button
                ...
                onClick={() => { setActiveFilter(f.key); setHl(0) }}
              >
        ...
        {sections.length === 0 ? (
          ...
          sections.map(section => (
            <Fragment key={section.key}>
              <div className="cl-search-section">{section.title} <span className="ct">· {section.count}</span></div>
              {section.rows.map(row => row.kind === 'project' ? (
                <ProjectSearchItem
                  key={row.key}
                  row={row}
      ...
      <div className="cl-search-foot">
        ...
        {mode === 'projects' && currentProject && onDeleteCurrent && (
          <button
            ...
            onClick={() => { onDeleteCurrent(currentProject); onClose() }}
          >
            Remove current
          </button>
        )}
...
function ProjectSearchItem({
  row, flatIdx, hl, setHl, onTogglePin, onSelect,
}: {
  row: ProjectRow
  flatIdx: number
  hl: number
  ...
  return (
    ...
    >
      ...
      <span className="pmeta"><b>{row.meta.split(' ')[0]}</b> {row.meta.split(' ').slice(1).join(' ')}</span>
      <button
        type="button"
        ...
        onClick={e => { e.stopPropagation(); onTogglePin(row.project.hash) }}
      >
