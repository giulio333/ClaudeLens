  ...
  return (
    <div className="cl-entity-v2" style={entityTint(config.color, { neutral: config.neutralTint })}>
      <section className="cl-entity-v2-hero">
        ...
        >
          {config.name}
          {config.titleGlyph && <span className="ext">{config.titleGlyph}</span>}
        </h1>
        {config.description && <div className="desc">{config.description}</div>}
        {config.path && (
          <div className="filepath" title={config.path}>
            {config.path}
          </div>
        )}
      </section>
      ...
      <section className={'cl-entity-v2-body' + (hasAside ? ' has-aside' : '')}>
        <div className="cl-entity-v2-main">
          <div className="lab">
            {config.bodyLabel} · {charCount} chars · {wordCount} words
          </div>
          <div className="prose">
            {body ? (
              <Markdown>{body}</Markdown>
            ) : (
              <p style={{ color: 'var(--cl-ink-3)', fontStyle: 'italic' }}>
            ...
            {hasOptions && (
              <div className="cl-entity-v2-opts">
                <h3>
                  ...
                  <b>
                    {setProperties.length} / {config.optionDefs.length} set
                  </b>
                </h3>
...
function EditWorkbench({
  ...
  const unsetDefs = config.optionDefs.filter(
    d => !d.required && !isOptionSet(state.options[d.key])
  );
  const hasLeftCol =
    !!config.hasDescriptionField || config.optionDefs.length > 0 || !!config.editExtras;
  const colorForTint =
    config.color ?? (typeof state.options.color === 'string' ? state.options.color : undefined);
  ...
  return (
    ...
    >
        ...
        <div className="crumbs">
          ...
          {config.path && (
            <>
              ...
              <span className="path" title={config.path}>
                {config.path}
              </span>
            </>
          )}
      ...
      <section className="cl-entity-edit-v2-head">
        <span className={'cl-entity-edit-v2-orb' + (config.neutralTint ? ' outline' : '')}>
          {initial}
        </span>
        <div style={{ minWidth: 0 }}>
          ...
          <div className="label-row">
            <span className="ey">
              <span className="pip" />
              Edit {config.kindLabel} · {config.scopeLabel}
            </span>
            <span style={{ opacity: 0.4 }}>·</span>
      ...
      <div className="cl-entity-edit-v2-work">
            ...
            {config.optionDefs.length > 0 && (
              <>
                <h2>
                  <span>Properties</span>
                  <b>
                    {setDefs.length} / {config.optionDefs.length} optional set
                  </b>
                </h2>
        ...
        <div className="cl-entity-edit-v2-col">
          <h2 className="title">
            <span>{config.kind === 'agent' ? 'System prompt' : 'Body'}</span>
            <b>markdown body</b>
          </h2>
          ...
          <div className="cl-entity-edit-v2-body">
            ...
            <textarea
              ...
              value={state.body}
              onChange={e => {
                onChange({ ...state, body: e.target.value });
                updatePos();
              }}
              onKeyUp={updatePos}
              onClick={updatePos}
            ...
            <div className="bf">
              ...
              <span>
                line <b>{pos.line}</b> col <b>{pos.col}</b>
              </span>
              <span className="grow" />
              <span>md · utf-8</span>
            </div>
    ...
    <div style={{ padding: '14px 56px 0' }}>
      ...
      >
        ...
        {validation.messages.map((m, i) => (
          <p key={i} style={{ fontSize: 13.5, lineHeight: 1.55, color: 'var(--cl-ink-2)' }}>
            {m}
          </p>
        ))}
      </div>
  ...
  return (
    <div className="cl-run-agent-backdrop" onClick={onCancel}>
      <div className="cl-run-agent-panel" onClick={e => e.stopPropagation()}>
        ...
        <h2>
          Delete <span style={{ color: 'var(--cl-accent)' }}>{name}</span>?
        </h2>
        ...
        <div className="actions">
          <button type="button" className="cl-btn-ghost" onClick={onCancel} disabled={busy}>
            Cancel
          </button>
          <button
            type="button"
        ...
        <>
          ...
          {canRun && (
            <button
              ...
              title="Dispatch as a background agent"
            >
              <PlayIcon /> Run agent
            </button>
          )}
