...
export function entityTint(color?: string, opts?: { neutral?: boolean }): CSSProperties {
  ...
  if (m)
    return {
      '--agent-h': String(m.h),
      '--agent-c': String(m.c),
      '--ident-tint': `oklch(0.62 ${m.c} ${m.h})`,
    } as CSSProperties;
  if (opts?.neutral)
    return {
      '--agent-h': '40',
      '--agent-c': '0',
      '--ident-tint': 'var(--cl-accent)',
    } as CSSProperties;
  return {
    '--agent-h': String(COLOR_MAP.green.h),
    '--agent-c': String(COLOR_MAP.green.c),
    '--ident-tint': `oklch(0.62 ${COLOR_MAP.green.c} ${COLOR_MAP.green.h})`,
  } as CSSProperties;
}
...
export const AGENT_OPTION_DEFS: OptionDef[] = [
  {
    key: 'color',
    label: 'color',
    frontmatterKey: 'color',
    blurb:
      'Accent color for the agent identity: red · orange · yellow · green · cyan · blue · purple · pink.',
    enum: ['green', 'red', 'orange', 'yellow', 'cyan', 'blue', 'purple', 'pink'],
  },
  {
    key: 'model',
    label: 'model',
    frontmatterKey: 'model',
    blurb: 'Model alias or full model ID. Omit to inherit from the current session.',
  },
  {
    key: 'allowedTools',
    label: 'tools',
    frontmatterKey: 'tools',
    blurb: 'Tools the subagent can use. Inherits all if omitted.',
    isArray: true,
    isTools: true,
  },
  {
    ...
    frontmatterKey: 'disallowedTools',
    blurb: 'Tools to deny, removed from the inherited list.',
    isArray: true,
    isTools: true,
  },
  {
    ...
    frontmatterKey: 'permissionMode',
    blurb: 'default · acceptEdits · auto · dontAsk · bypassPermissions · plan',
    enum: ['default', 'acceptEdits', 'auto', 'dontAsk', 'bypassPermissions', 'plan'],
  },
  {
    key: 'maxTurns',
    label: 'maxTurns',
    frontmatterKey: 'maxTurns',
    blurb: 'Maximum agentic turns before the subagent stops.',
    isNumber: true,
  },
  {
    key: 'isolation',
    label: 'isolation',
    frontmatterKey: 'isolation',
    blurb: 'Set to worktree to run in an isolated git worktree.',
    enum: ['worktree'],
  },
  {
    key: 'memory',
    label: 'memory',
    frontmatterKey: 'memory',
    blurb: 'Persistent memory scope: user · project · local.',
  ...
  },
  {
    ...
    blurb: 'Skills preloaded at startup, full content injected.',
    isArray: true,
  },
  {
    key: 'mcpServers',
    label: 'mcpServers',
    frontmatterKey: 'mcpServers',
    blurb: 'MCP servers available to this subagent.',
    isArray: true,
  },
  {
    key: 'effort',
    ...
    blurb: 'Effort level: low · medium · high · xhigh · max.',
    enum: ['low', 'medium', 'high', 'xhigh', 'max'],
  },
  {
    key: 'background',
    label: 'background',
    frontmatterKey: 'background',
    blurb: 'Always run this subagent as a background task.',
    isBool: true,
  },
  {
    key: 'disableModelInvocation',
    label: 'disable_model_invocation',
    frontmatterKey: 'disable_model_invocation',
    blurb: 'Hide this subagent from automatic invocation.',
    isBool: true,
  },
];
...
export const SKILL_OPTION_DEFS: OptionDef[] = [
  {
    ...
    blurb: 'Hint shown in autocomplete — e.g. [filename] [format].',
  },
  {
    key: 'allowedTools',
    label: 'allowed-tools',
    frontmatterKey: 'allowed-tools',
    blurb: 'Tools Claude can use without requesting permission when the skill is active.',
    isArray: true,
    isTools: true,
  },
  {
    ...
    label: 'model',
    frontmatterKey: 'model',
    blurb: 'Model to use when the skill is active — e.g. claude-sonnet-4-6.',
  },
  {
    key: 'context',
    label: 'context',
    frontmatterKey: 'context',
    blurb: 'Set to fork to run in an isolated forked subagent.',
    enum: ['fork'],
  },
  {
    key: 'agent',
    ...
    blurb: 'Type of subagent to use when context: fork is set.',
  },
  {
    key: 'disableModelInvocation',
    label: 'disable-model-invocation',
    frontmatterKey: 'disable-model-invocation',
    blurb: 'If true, Claude does not load the skill automatically — must be invoked with /name.',
    isBool: true,
  },
  {
    key: 'userInvocable',
    ...
    blurb: 'If false, the skill is hidden from the / menu — used only as background knowledge.',
    isBool: true,
  },
];
...
export const MEMORY_OPTION_DEFS: OptionDef[] = [
  {
    ...
    label: 'type',
    frontmatterKey: 'type',
    blurb: 'Memory type: user · feedback · project · reference.',
...
export function serializeMemory(
  ...
  const type =
    typeof opts.options.type === 'string' && opts.options.type ? opts.options.type : topic.type;
