...
export function TopBar({ onBack, crumb, accent }: { onBack: () => void; crumb: string; accent: Accent }) {
  const hoverCls = accent === 'violet' ? 'hover:text-[var(--cl-violet)]' : 'hover:text-[var(--cl-accent)]'
  return (
    <div
    ...
    >
      <button
        onClick={onBack}
        className={`flex items-center gap-1.5 font-mono uppercase transition-colors ${hoverCls}`}
