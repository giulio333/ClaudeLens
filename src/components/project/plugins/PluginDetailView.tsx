...
export function PluginDetailView({
  ...
  const plugin =
    plugins?.find(
      p => p.marketplace === initialPlugin.marketplace && p.name === initialPlugin.name
    ) ?? initialPlugin;
  ...
  return (
    <div className="h-full flex flex-col" style={{ background: 'var(--cl-paper)' }}>
      <TopBar
        onBack={onBack}
        crumbs={[{ label: 'Plugins' }, { label: plugin.name, accent: true }]}
      />
            ...
            <div className="cl-tile-grid cl-tile-grid--list">
              {plugin.skills.map((s, i) => (
                <Tile
                  ...
                  desc={s.description}
                  index={i}
                  onClick={() => setOpen({ kind: 'skill', item: s })}
                />
              ))}
            </div>
          </section>
            ...
            <div className="cl-tile-grid cl-tile-grid--list">
              {plugin.agents.map((a, i) => (
                <Tile
                  ...
                  desc={a.description}
                  index={i}
                  onClick={() => setOpen({ kind: 'agent', item: a })}
                />
              ))}
            </div>
          </section>
          ...
          <section className="cl-section">
            ...
            <div className="cl-tile-grid cl-tile-grid--list">
              {plugin.commands.map((c, i) => (
                <Tile
                  key={c.path}
                  glyph={(c.name[0] ?? '?').toUpperCase()}
                  name={`/${c.name}`}
                  desc={c.description}
                  index={i}
                  onClick={() => setOpen({ kind: 'command', item: c })}
                />
              ))}
            </div>
          </section>
