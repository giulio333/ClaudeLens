...
function componentCounts(plugin: InstalledPlugin): string {
  const parts: string[] = []
  if (plugin.skills.length) parts.push(`${plugin.skills.length} skill${plugin.skills.length === 1 ? '' : 's'}`)
...
function PluginTile({ plugin, index, onClick }: { plugin: InstalledPlugin; index: number; onClick: () => void }) {
  return (
    <button type="button" className={`cl-tile ${index === 0 ? 'accent' : ''}`} onClick={onClick}>
      ...
      <span className="t-meta"><b>{componentCounts(plugin)}</b></span>
    </button>
  )
}
      ...
      <div className="flex-1 overflow-y-auto">
        <section className="cl-hero">
          ...
          <div className="cl-h-meta">
            <span><b>{total}</b> {total === 1 ? 'plugin' : 'plugins'}</span>
          ...
          [...byMarketplace.entries()].map(([marketplace, list]) => (
            <section key={marketplace} className="cl-section">
              <div className="cl-sec-head">
                <h2>{marketplace}</h2>
                <span className="ct">{list.length} {list.length === 1 ? 'plugin' : 'plugins'}</span>
              </div>
              <div className="cl-tile-grid cl-tile-grid--list">
                {list.map((p, i) => (
                  <PluginTile key={`${p.marketplace}/${p.name}`} plugin={p} index={i} onClick={() => onSelectPlugin(p)} />
                ))}
              </div>
            </section>
          ))
