...
export function GlobalSkillsView({
  ...
  return (
    <div className="h-full flex flex-col" style={{ background: 'var(--cl-paper)' }}>
      <TopBar
        onBack={onBack}
        crumbs={[{ label: project ? `Project · Skills · ${projectName}` : 'Global · Skills' }]}
      />
      ...
      <div className="flex-1 overflow-y-auto">
        <section className="cl-hero">
          ...
          <div className="cl-eyebrow">
            <span className="pip" />
            <span>
              {project ? `Project · ${projectName} · skills` : 'Global · ~/.claude/skills'}
            </span>
          </div>
          ...
          <div className="cl-h-meta">
            <span>
              <b>{total}</b> {total === 1 ? 'skill' : 'skills'}
            </span>
            ...
            {project && onlyGlobalSkills.length > 0 && (
              <>
                <span className="sep">·</span>
                <span>
                  <b>{projectSkills.length}</b> project · <b>{onlyGlobalSkills.length}</b> global
                </span>
              </>
            )}
          </div>
        ...
        {isLoading ? (
          ...
          <section className="cl-section">
            <div className="cl-empty">
              No skills found. Add <code style={{ fontFamily: 'var(--font-mono)' }}>*.md</code>{' '}
              files in <code style={{ fontFamily: 'var(--font-mono)' }}>~/.claude/skills/</code>.
            </div>
          </section>
        ) : project ? (
          <>
