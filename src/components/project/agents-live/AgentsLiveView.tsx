...
import {
  ...
  useDeleteBackgroundAgent,
  useStopBackgroundAgent,
  useRespawnBackgroundAgent,
  useAttachBackgroundAgent,
} from '../../../hooks/useIPC';
...
function ActionCluster({ s, actions }: { s: BgSession; actions: RowActions }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 4 }} onClick={stop}>
      <ActionButton
        label="attach"
        title="Attach in Terminal (claude attach)"
        onClick={e => {
          e.stopPropagation();
          actions.onAttach();
        }}
      ...
      <ActionButton
        label="stop"
        title={canStop ? 'Stop the running session (claude stop)' : 'Session is not running'}
      ...
      <ActionButton
        label="restart"
        title="Restart session, preserving conversation (claude respawn)"
        onClick={e => {
          e.stopPropagation();
          actions.onRestart();
        }}
...
function DispatchSelect({
  ...
  return (
    <span className="cl-composer-select" ref={rootRef}>
      ...
      >
        {icon && (
          <span className="cl-composer-chip-icon" aria-hidden>
            {icon}
          </span>
        )}
        {current?.label ?? value}
...
export function AgentsLiveView({
  ...
  const allAgents = [...projectAgents, ...globalAgents].filter(
  ...
  const [busyIds, setBusyIds] = useState<
    Record<string, { stop?: boolean; restart?: boolean; remove?: boolean; attach?: boolean }>
  >({});
  ...
  const liveCount = sessions.filter(
    s => s.alive && !TERMINAL_BUCKETS.has(statusOf(s).bucket)
  ).length;
  ...
  const groups = BUCKET_ORDER.map(b => ({
    ...b,
    items: sessions.filter(s => statusOf(s).bucket === b.key),
    ...
    >
      ...
      {!embedded && (
        <TopBar
          onBack={onBack}
          crumbs={[
            { label: project ? `Project · Agent View · ${projectName}` : 'Global · Agent View' },
          ]}
        />
      )}
        ...
        {!hideHero && (
          <section className={`cl-hero${liveCount > 0 ? ' is-live' : ''}`}>
            ...
            <div className="cl-eyebrow">
              ...
              <span>
                {project
                  ? `Project · ${projectName} · background sessions`
              ...
              <section className="cl-section" key={g.key}>
                ...
                <div className="cl-tile-grid cl-tile-grid--list">
                  {g.items.map(s => {
                    ...
                    const handle = (
                      key: 'stop' | 'restart' | 'remove' | 'attach',
                      run: () => Promise<unknown>
                    ) => {
                    ...
                    return (
                      <SessionRow
                        ...
                        actions={{
                          onAttach: () =>
                            handle('attach', () => attachBg.mutateAsync({ cwd: s.cwd, id: s.id })),
          ...
          >
            ...
            <div className="cl-dispatch-card">
              <div className="cl-dispatch-prompt">
                <span className="cl-dispatch-glyph" aria-hidden>
                  ⌁
                </span>
                <textarea
                  ...
                  onKeyDown={e => {
                    if (e.key === 'Enter' && !e.shiftKey) {
                      e.preventDefault();
                      if (prompt.trim() && !dispatchBg.isPending)
              ...
              <div className="cl-dispatch-foot">
                <div className="cl-dispatch-controls">
                  ...
                  <label className="cl-dispatch-name-wrap">
                    <span className="cl-dispatch-name-icon" aria-hidden>
                      ✎
                    </span>
                    <input
                      type="text"
