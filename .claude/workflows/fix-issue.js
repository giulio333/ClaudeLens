export const meta = {
  name: 'fix-issue',
  description:
    'Pick an open GitHub issue, implement the fix on a branch and open a PR that closes it',
  whenToUse:
    'When the user wants an open GitHub issue picked up and carried through to a pull request',
  phases: [
...
if (!picked || !picked.number) {
  return {
    error:
      'Fase Pick non conclusa (agente interrotto o nessuna issue adatta): rilancia, oppure forza con args.issue.',
  };
}
...
if (!fix || !fix.green || !fix.changedFiles.length) {
  return {
    ...
    branch,
    fix,
    result: 'Verifica non verde: modifiche lasciate sul branch, nessuna PR aperta.',
  };
}
...
return {
  issue: { number: picked.number, title: picked.title },
  branch,
