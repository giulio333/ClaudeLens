export const meta = {
  name: 'update-tests',
  description:
    'Create or update the test suite of a repository: map the codebase, find coverage gaps, write/refresh tests per module, then verify',
  whenToUse:
    'When the user asks to create missing tests or bring an existing test suite up to date with the current code',
  phases: [
    {
      title: 'Map',
      detail: 'understand the repo, test framework and existing tests',
      model: 'sonnet',
    },
...
if (!items.length)
  return { repoMap, result: 'La suite è già allineata: nessun modulo da lavorare.' };
...
Prima di chiudere esegui SOLO i test di questo modulo con il runner del repo (comando base: ${repoMap.testCommand}) e sistemali finché passano.`,
  ...
  (result, item) =>
    result ? { ...result, sourcePath: item.sourcePath, action: item.action } : null
);
